...
class BWFAN_WasapBom_Send_Media extends BWFAN_Action {

    private static $instance = null;
    private bool $progress = false;

    public function __construct() {
        $this->action_name = __( 'Send Media Message', 'funnelkit-wasapbom' );
        $this->action_desc = __( 'This action sends a media message (image, video, document) via WasapBom', 'funnelkit-wasapbom' );
        $this->support_v2  = true;
    }

    /**
     * @return BWFAN_WasapBom_Send_Media|null
     */
    ...
    public function make_v2_data( $automation_data, $step_data ): array
    {
        ...
        $data_to_set = array(
            ...
            'media_url'       => isset( $step_data['media_url'] ) ? BWFAN_Common::decode_merge_tags( $step_data['media_url'] ) : '',
            'mediatype'       => isset( $step_data['mediatype'] ) ? $step_data['mediatype'] : 'image',
            'caption'         => isset( $step_data['caption'] ) ? BWFAN_Common::decode_merge_tags( $step_data['caption'] ) : '',
        );
        ...
        if ( isset( $data_to_set['promotional_sms'] ) && 0 === absint( $data_to_set['promotional_sms'] ) ) {
            $data_to_set['caption'] = str_replace( '{{unsubscribe_link}}', '', $data_to_set['caption'] );
        }

        $data_to_set['caption'] = stripslashes( $data_to_set['caption'] );
    ...
    public function execute_action( $action_data ): array
    {
        ...
        if ( is_null( $call_class ) ) {
            ...
            return array(
                'status'  => 4,
                'message' => __( 'Send Media call not found', 'funnelkit-wasapbom' ),
            );
        }
        ...
        $conversation  = $this->create_engagement( $this->data['number'], $automation_id, $this->data['caption'] );
        if ( $conversation instanceof BWFAN_Engagement_Tracking ) {
            $this->data['caption'] = $this->add_tracking_code( $conversation, $this->data['caption'] );
        }
        ...
        if ( is_array( $response ) && 200 === $response['response'] && ( isset( $response['body']['success'] ) && $response['body']['success'] === true ) ) {
            ...
            return array(
                'status'  => 3,
                'message' => __( 'Media message sent successfully.', 'funnelkit-wasapbom' ),
            );
        }
    ...
    public function handle_response_v2( $response ) {
        ...
        if ( is_array( $response ) && 200 === $response['response'] && ( isset( $response['body']['success'] ) && $response['body']['success'] === true ) ) {
            ...
            return $this->success_message( __( 'Media message sent successfully.', 'funnelkit-wasapbom' ) );
        }
    ...
    public function get_fields_schema(): array
    {
        return [
            ...
            [
                'id'          => 'mediatype',
                'label'       => __( "Media Type", 'funnelkit-wasapbom' ),
                'type'        => 'radio',
                'options'     => [
                    [
                        'label' => __( "Image", 'funnelkit-wasapbom' ),
                        'value' => 'image',
                    ],
                    [
                        'label' => __( "Video", 'funnelkit-wasapbom' ),
                        'value' => 'video',
                    ],
                    [
                        'label' => __( "Document", 'funnelkit-wasapbom' ),
                        'value' => 'document',
                    ],
                ],
                "description" => '',
            ...
            [
                'id'          => 'media_url',
                'label'       => __( "Media URL", 'funnelkit-wasapbom' ),
                'type'        => 'text',
                'placeholder' => "https://example.com/image.jpg",
                "class"       => 'bwfan-input-wrapper',
                'tip'         => __( 'Direct URL to the media file', 'funnelkit-wasapbom' ),
                "description" => '',
                "required"    => true,
            ],
            [
                'id'          => 'caption',
                'label'       => __( "Caption (Optional)", 'funnelkit-wasapbom' ),
                'type'        => 'textarea',
            ...
            [
                ...
                'type'        => 'send_data',
                'label'       => '',
                'send_action' => 'bwfan_wb_test_media',
                'send_field'  => [
...
return 'BWFAN_WasapBom_Send_Media';